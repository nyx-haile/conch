use conch::interview::speculative::{
    partial_is_stable_long_enough, should_commit_draft, similarity,
};
...
fn short_prefix_does_not_auto_commit() {
    ...
    assert!(!should_commit_draft(
        "so",
        "so after thinking about it I took a different approach"
    ));
}
