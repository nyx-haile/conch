...
#[tokio::test]
async fn deepgram_stt_uses_launch_realtime_query_options() {
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let port = listener.local_addr().unwrap().port();
    let url = format!("ws://127.0.0.1:{}/v1/listen", port);
    ...
    let server = tokio::spawn(async move {
        ...
        let mut ws = tokio_tungstenite::accept_hdr_async(
            ...
            move |req: &http::Request<()>,
                  resp: http::Response<()>|
                  -> Result<http::Response<()>, http::Response<Option<String>>> {
                *query_clone.lock().unwrap() = req.uri().query().map(|s| s.to_string());
                Ok(resp)
            },
        )
        .await
    ...
    let query = server.await.unwrap().unwrap();
    assert!(query.contains("encoding=linear16"), "query={query}");
    assert!(query.contains("sample_rate=16000"), "query={query}");
    assert!(query.contains("model=nova-3"), "query={query}");
    assert!(query.contains("interim_results=true"), "query={query}");
    assert!(query.contains("endpointing=350"), "query={query}");
    assert!(query.contains("punctuate=true"), "query={query}");
