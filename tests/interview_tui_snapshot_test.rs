...
fn make_state() -> AppState {
    ...
    s.push_turn(TurnView { speaker: Speaker::Conch, text: "So, what made you build this?".into() });
    s.push_turn(TurnView { speaker: Speaker::User, text: "Wanted to learn audio.".into() });
    s
}
...
fn renders_listening_state_without_panic() {
    ...
    term.draw(|f| render_frame(f, &st, Duration::from_secs(42))).unwrap();
    let buf = term.backend().buffer().clone();
    let dump = buffer_text(&buf);
    assert!(dump.contains("Listening"), "expected 'Listening' in:\n{dump}");
    assert!(dump.contains("00:00:42"), "expected '00:00:42' in:\n{dump}");
    assert!(dump.contains("conch talk"), "expected 'conch talk' in:\n{dump}");
    assert!(dump.contains("Brief"), "expected 'Brief' in:\n{dump}");
}

#[test]
fn renders_thinking_and_speaking_states() {
    for status in [Status::Thinking, Status::Filling, Status::Speaking, Status::Filler] {
        let backend = TestBackend::new(100, 30);
        let mut term = Terminal::new(backend).unwrap();
        let mut st = make_state();
        st.set_status(status);
        st.update_assistant_stream("Interesting, let me think");
        term.draw(|f| render_frame(f, &st, Duration::from_secs(1))).unwrap();
    }
}
