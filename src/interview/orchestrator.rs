...
type PendingReplyHandle = tokio::task::JoinHandle<Result<(String, Vec<ToolCall>)>>;

struct PendingReply {
    user_text: String,
    speculative_partial: String,
    speculative_handle: Option<PendingReplyHandle>,
}
...
impl Orchestrator {
    ...
    async fn run_inner(&mut self) -> Result<EndSignal> {
        ...
        loop {
            ...
            match event {
                UserEvent::Quit => return self.finish_user_quit().await,
                UserEvent::Interrupt => {
                    // Nothing playing at idle — ignore.
                    continue;
                }
                UserEvent::MicToggle => {
                    if let Some(signal) = self.handle_mic_toggle().await? {
                        return Ok(signal);
                    }
                }
            }
    ...
    async fn opening_turn(&self) -> Result<String> {
        ...
        self.chat_for_text(seed, vec![]).await
    }

    async fn handle_mic_toggle(&mut self) -> Result<Option<EndSignal>> {
        let mut pending = match self.capture_user_turn().await? {
            RecordOutcome::Quit => return self.finish_user_quit().await.map(Some),
            RecordOutcome::NoSpeech => return Ok(None),
            RecordOutcome::Captured(pending) => pending,
        };

        self.commit_user_turn(&pending.user_text).await;

        if detect_end_command(&pending.user_text) {
            pending.abort_speculative();
            let signal = self
                .finish_with_generated_closing(
                    "The user wants to wrap up. Give a brief, warm closing.",
                    EndSignal::VoiceCommand,
                )
                .await?;
            return Ok(Some(signal));
        }

        let (reply, tool_calls) = self.resolve_reply(&mut pending).await?;
        if let Some(reason) = Self::extract_end_session(&tool_calls) {
            ...
            let signal = self
                .finish_with_closing_text(closing_text, EndSignal::LlmEndSession { reason })
                .await?;
            return Ok(Some(signal));
        }
        ...
        if self.speak_interruptible(&reply).await? == SpeakOutcome::Quit {
            return self.finish_user_quit().await.map(Some);
        }
        Ok(None)
    }

    async fn capture_user_turn(&mut self) -> Result<RecordOutcome> {
        self.begin_record_phase().await;
        ...
        let mut speculative_handle: Option<PendingReplyHandle> = None;
        ...
        if user_text.is_empty() {
            ...
            return Ok(RecordOutcome::NoSpeech);
        }

        Ok(RecordOutcome::Captured(PendingReply {
            user_text,
            speculative_partial,
            speculative_handle,
        }))
    ...
    async fn begin_record_phase(&self) {
        let mut st = self.state.write().await;
        st.set_banner(None);
        st.set_status(Status::Listening);
    }

    async fn end_record_phase(&self) {
        if let Some(gate) = &self.mic_gate {
            gate.set_open(false);
        }
        let mut st = self.state.write().await;
    ...
    async fn resolve_reply(
        ...
        let speculative_result = if let Some(handle) = pending.speculative_handle.take() {
            if should_commit_draft(&pending.speculative_partial, &pending.user_text) {
                match handle.await {
    ...
    async fn finish_with_generated_closing(
        &mut self,
        prompt: &str,
        signal: EndSignal,
    ) -> Result<EndSignal> {
        let mut closing_msgs = self.messages.clone();
        closing_msgs.push(Message::user(prompt));
        let closing = self.chat_for_text(closing_msgs, vec![]).await?;
        self.finish_with_closing_text(closing, signal).await
    }

    async fn finish_with_closing_text(
        &mut self,
        closing_text: String,
        signal: EndSignal,
    ) -> Result<EndSignal> {
    ...
    async fn finish_user_quit(&mut self) -> Result<EndSignal> {
        self.set_status(Status::Closing).await;
        Ok(EndSignal::UserQuit)
    }
